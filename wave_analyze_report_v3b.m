...
normalize_flag = 0;
resample_rate = 44100;  % when exporting, set to this rate for audio files
% report formatting
rows_per_page = 4;
report_prefix = 'wave_effects';  % for pdf out
%exported audio file parameters
...
for k = 1:num_files
    ...
    audiowrite(out_filename,w_data,resample_rate);
end
...
pos_y = 0.5;
%figure printing
close all;  % close all figure windows that are already open
...
% this doesn't work yet (ghostscript cmd line error); I used PDFtk for now.   append_pdfs('report.pdf', out_filename_array{:} );
