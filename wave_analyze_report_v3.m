...
% name date size SR   max  min  mean
% 1    2    3    4    5    6    7
wav_attrib_headings = { 'name' 'date' 'size' 'SR  ' 'max ' 'min ' 'mean' };
...
for k = 1:num_files
    ...
    w_data = single(w_data);    % reduce amt of data to single precision
    % option to normalize
    if normalize_flag == 1
        mx = max(w_data);
        mn = min(w_data);
        ...
        scale = abs(mx);    % find peak
        wav_norm = w_data/scale;
        
        w_data = wav_norm ; % overwrite original var
        strcat(report_prefix,'_N');
    end
    num_secs = 2;
    Lmax = 44100 * num_secs;
    ...
    wav_data(k,1:L) = single(w_data(1:L));
    ...
    wav_attrib_str(k,2) = wav_attributes(k,2);
    
    wav_attrib_str(k,3) = wav_attributes(k,3);
    wav_attrib_str{k,3} = strcat(num2str(wav_attrib_str{k,3}/1000),' kB');
    
    wav_attrib_str(k,4) = wav_attributes(k,4);
    wav_attrib_str{k,4} = strcat(num2str(wav_attrib_str{k,4}/1000),' kHz');
    
    wav_attrib_str(k,5) = wav_attributes(k,5);
    wav_attrib_str{k,5} = strcat(num2str(wav_attrib_str{k,5}/1.0 ),' max');
...
out_filename = fullfile(thepath,theoutfolder,fn);
    audiowrite(out_filename,w_data,file_sample_rate);
end
...
for k = 1:num_files
    ...
    xlabel('f (Hz)');
    % ylabel('|P1(f)|');
    axis([0 400 0.01-0.001 1.0]); %inf]);  % values to 300Hz, log scaling  to 1:100 max amplitude
    set(gca, 'YScale', 'log');
    grid on;
...
for k = 1 : num_pages
    ...
    saveas(gcf,fn,filetype);   % automatically adds extension
end
